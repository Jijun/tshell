...
Format:
  ./tshell config init [-c <config-file-path>]
...
func initConfigFile(cfgFlag bool) {
	...
	if cfgFlag {
		fmt.Println("Specify the path of the configuration file: (default:" + home + "/.tshell.yaml)")
		_, _ = fmt.Scanf("%s\n", &configFile)
	}
	if configFile == "" {
		configFile = home + "/.tshell.yaml"
	}
	if configFile[0] == '~' {
	...
	viper.Set("tshell", config)

	if err := viper.WriteConfigAs(configFile); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	fmt.Printf("\nThe configuration file is initialized successfully! \nYou can use \"./tshell config show [-c <Config File Path>]\" show the contents of the specified configuration file\n")
}
