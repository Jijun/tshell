...
Format:
  ./tshell sync <source_path> <destination_path> [flags]
...
func init() {
	...
	syncCmd.Flags().String("snapshot-path", "", "This option is used to accelerate the incremental"+
		...
		" tshell will generate files to record the snapshot information in the specified directory."+
		" When the next time you upload files or download objects with the option, "+
		"tshell will read the snapshot information under the specified directory for incremental upload or incremental download. "+
		"The snapshot-path you specified must be a local file system directory can be written in, "+
		"if the directory does not exist, tshell creates the files for recording snapshot information, "+
		"else tshell will read snapshot information from the path for "+
		"incremental upload(tshell will only upload the files which haven't not been successfully uploaded to oss or"+
		" been locally modified) or incremental download(tshell will only download the objects which have not"+
		...
		"In addition, tshell does not automatically delete snapshot-path snapshot information, "+
